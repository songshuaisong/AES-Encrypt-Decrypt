...
void encipherment(uint8_t *in, uint8_t *out, uint8_t *w)
{
	uint8_t state[4 * Nb];
	uint8_t r, i, j;
...
void decipherment(uint8_t *in, uint8_t *out, uint8_t *w)
{
...
{
	if ((encrypt == NULL) || (encrypted == NULL) || (key == NULL) || (num % 16) != 0)
	{
		return -1;
	}
	...
	for (i = 0; i < num / 16; i++)
	{
		encipherment(&encrypt[16 * i], &encrypted[16 * i], w);
	}
	free(w);
	return 0;
}
...
{
	if ((decrypt == NULL) || (decrypted == NULL) || (key == NULL) || (num % 16) != 0)
	{
		return -1;
	}
	...
	for (i = 0; i < num / 16; i++)
	{
		decipherment(&decrypt[16 * i], &decrypted[16 * i], w);
	}
	free(w);
	return 0;
}
...
{
	int i;
	if ((encdec->src == NULL) || (encdec->dst == NULL) || ((encdec->srclen % 16) != 0))
	{
		return -1;
	}

	uint8_t *w; // expanded key
	switch (encdec->keylen)
	{
	case 16:
		Nk = 4;
		Nr = 10;
	...
	w = malloc(Nk * (Nr + 1) * 4);
	key_expansion(encdec->key, w);

	for (i = 0; i < encdec->srclen / 16; i++)
	{
		encipherment(&encdec->src[16 * i], &encdec->dst[16 * i], w);
	}
	free(w);
	return 0;
}
...
int user_decrypt(encdec_t *encdec)
{
	...
	for (i = 0; i < encdec->srclen / 16; i++)
	{
		decipherment(&encdec->src[16 * i], &encdec->dst[16 * i], w);
	}
	free(w);
	return 0;
}
